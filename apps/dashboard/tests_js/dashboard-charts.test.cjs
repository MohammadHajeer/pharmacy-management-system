...
function page({ library = true, source = true, invalid = false, fail = false, hasData = true, chart = {} } = {}) {
  ...
  const context = vm.createContext({
    ...
    getComputedStyle: () => ({ getPropertyValue: (token) => token, fontFamily: "sans-serif" }),
    Intl, console: { warn() {} },
  });
...
test("stock is three vertical semantic columns with no pastel fills or tracks", () => {
  ...
  assert.deepEqual(Array.from(bars.backgroundColor), ["--color-primary-600", "--color-warning", "--color-danger"]);
  ...
  assert.equal(config.options.plugins.tooltip.borderColor, "--color-slate-700");
...
test("inventory value labels include zeroes and stay inside the canvas", () => {
  ...
  assert.equal(chart.ctx.fillStyle, "--color-slate-700");
...
test("purchase columns are wider muted teal with a deep-teal featured month", () => {
  ...
  assert.deepEqual(Array.from(config.data.datasets[0].backgroundColor), ["--color-primary-700", "--color-primary-700", "--color-primary-800"]);
  const labels = [];
...
test("expiry isolates urgent buckets without changing counts or using a distorted scale", () => {
  for (const chart of [
    ...
    assert.deepEqual(Array.from(config.data.datasets[0].backgroundColor), chart.tones.length === 4
      ? ["--color-danger", "--color-warning", "--color-slate-500"] : ["--color-danger", "--color-warning"]);
