...
# Phase 1 presentation data. Replace these records with dashboard query results
# without changing the permission-aware context shape consumed by the template.
SAMPLE_KPIS = (
    {
        "label": "Today's Sales",
        "value": "$2,450.00",
...
def dashboard_view(request):
    return render(
        ...
        "dashboard/index.html",
        {
            ...
            "kpis": _visible_items(request.user, SAMPLE_KPIS),
            "recent_activity": _visible_items(
                request.user,
                SAMPLE_RECENT_ACTIVITY,
            ),
            "attention_items": _visible_items(
                request.user,
                SAMPLE_ATTENTION_ITEMS,
            ),
        },
